...
export async function handleAntispamCommand(sock, msg, args, sender) {
  ...
  if (param !== 'on' && param !== 'off') {
    ...
    return sock.sendMessage(from, { text: `⚠️ Uso: \`/antispam on\` ou \`/antispam off\`\n\nEstado atual: *${cfg.antispam ? 'ATIVADO' : 'DESATIVADO'}*` }, { quoted: msg });
  }

  const active = param === 'on' ? 1 : 0;
  updateGroupConfig(from, { antispam: active });

  return sock.sendMessage(from, { text: `🚫 Anti-spam foi *${active ? 'ATIVADO' : 'DESATIVADO'}* para este grupo!` }, { quoted: msg });
}
